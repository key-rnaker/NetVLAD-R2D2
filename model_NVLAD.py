...
class NetVLAD(nn.Module) :
    def __init__(self, clsts, traindescs) :
        ...
        del clsts, dsSq
        self.conv.weight = nn.Parameter( (2.0 * self.alpha * self.centroids).unsquuze(-1).unsquuze(-1))
    ...
    def forward(self, image) :
        ...
        soft_assign = self.conv(x).view(num_batchs, self.num_clusters, -1)
        soft_assign = F.softmax(soft_assign, dim=1)

        # local descriptor x flatten
        # x_flatten.shape = [batch갯수,H x W, encoder_dim = 512]
        # x_flatten = x.view(num_batchs, num_channels, -1)
        x_flatten = x.view(num_batchs, -1, num_channels)

        # vlad.shape = [batch갯수, num_clusters=64, num_channels=512]
        vlad = torch.zeros([num_batchs, self.num_clusters, num_channels], dtype=x.dtype, layout=x.layout, device=x.device)
        ...
        vlad = vlad.view(num_batchs, -1)     # flatten
        vlad = F.normalize(vlad, p=2, dim=1) # L2-normalization

        # vlad.shape = [num_batchs, num_clusters x num_channels]
